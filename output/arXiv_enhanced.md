...
![The image shows a screenshot of the arXiv.org website.  It displays a search bar, a list of subjects \(Physics and Mathematics are shown\), and a detailed breakdown of sub-categories within Physics, each with links to recent papers.  The site boasts open access to over 1.1 million e-prints.  There are also some announcements regarding user surveys and project updates. ](images/arXiv.pdf-0-0.png)
...
![The image shows a middle-aged man with short, graying brown hair and a beard. He's wearing a blue crewneck sweatshirt and appears to be speaking or gesturing with his hands.  The background is blurred but shows a muted orange wall. ](images/arXiv.pdf-1-0.png)
...
![The image shows a simple search bar on a dark red background.  There's a text field to enter a search query or article ID, a dropdown menu presumably for selecting search scope \(currently showing "All papers"\), and a "Go\!" button to initiate the search.  A "Login" link is also visible in the upper right corner, along with help and advanced search options.  The overall appearance is that of a basic website search interface. ](images/arXiv.pdf-1-1.png)
圖3：arXiv.org簡要檢索介⾯面

(⼆二)進階檢索：提供使⽤用者透過輸入或組合作者、篇名，或摘要等關鍵字查詢文章。

![The image shows a search form for a database of academic papers.  Users can search by author, title, or abstract, and can filter results by subject area \(e.g., Computer Science, Mathematics\) and publication year.  The form also allows specifying a range of years or selecting only the past year.](images/arXiv.pdf-2-0.png)

圖4：arXiv.org進階檢索介⾯面

(三)全文檢索：提供使⽤用者以關鍵字查詢全文。

![The image shows a simple search bar for an experimental full-text search.  It includes a text field for the search term, a dropdown menu \(currently set to "Everything"\), and a "Do Search" button.  Below the search bar is a disclaimer stating that the service is experimental and may be less up-to-date than the regular search, with a link to help documentation. ](images/arXiv.pdf-2-1.png)
